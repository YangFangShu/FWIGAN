...
class Propagator(deepwave.base.propagator.Propagator):
    ...
    def __init__(self, model, dx, pml_width=None, order=4, survey_pad=None, vpmax=None):
        ...
        if order%2 != 0:
            raise RuntimeError(
                "order must be even number, but the order is {}".format(order)
            )
        if order<2 or order>8:
            raise RuntimeError(
                "order must be greater than 2 and less than 8, but the order is {}".format(order)
            )
        if model["vp"].min() <= 0.0:
            raise RuntimeError(
...
class PropagatorFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        timestep = Timestep(dt, model.dx, max_vel, order)
        ...
        wavefield_save_strategy = _set_wavefield_save_strategy(
            ctx.needs_input_grad[7], dt, timestep.inner_dt, scalar_wrapper
        )
        fd1, fd2 = _set_finite_diff_coeffs(model.ndim, model.dx, device, dtype, order)
        ...
        model_gradient = _allocate_grad(vp, ctx.needs_input_grad[7])
    ...
    def backward(ctx, grad_receiver_amplitudes):
        ...
        return (source_gradient, None, None, None, None, None, None, model_gradient)
...
def _set_finite_diff_coeffs(ndim, dx, device, dtype, order):
    ...
    if order == 4:
        fd1_ = [8/12, -1/12, 0, 0]
        fd2_ = [-5/2, 4/3, -1/12, 0, 0]
    elif order == 6:
        fd1_ = [3/4, -3/20, 5/300, 0]
        fd2_ = [-245/90, 3/2, -3/20, 1/90, 0]
    elif order == 8:
        fd1_ = [4/5, -1/5, 4/105, -1/280]
        fd2_ = [-205/72, 8/5, -1/5, 8/315, -1/560]
    else:
        raise RuntimeError("order is wrong")
    ...
    for dim in range(ndim):
        fd1[dim] = (
            torch.tensor(fd1_, device=device, dtype=dtype)
            / dx[dim]
        )
        fd2[0] += fd2_[0] / dx[dim] ** 2
        fd2[1 + dim * 4 : 1 + (dim + 1) * 4] = (
            torch.tensor(fd2_[1:], device=device, dtype=dtype)
            / dx[dim] ** 2
        )
    return fd1, fd2
...
class Timestep(object):
    ...
    def __init__(self, dt, dx, max_vel, order):
        if order == 4:
            max_dt = 0.6124 / np.linalg.norm(1 / dx.numpy()) / max_vel
        elif order == 6:
            max_dt = 0.8135 / np.linalg.norm(1 / dx.numpy()) / max_vel / np.sqrt(2)
        elif order == 8:
            max_dt = 0.7774 / np.linalg.norm(1 / dx.numpy()) / max_vel / np.sqrt(2)
        else:
            raise NotImplementedError
